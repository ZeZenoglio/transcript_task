...
class Problem(BaseModel):
    ...
    instance: str | None = Field(
        default=None, description="The request path that produced this problem."
    )
...
class JobCreateResponse(BaseModel):
    job_id: str = Field(
        description="Also the transcript_id used everywhere else for this "
        "recording: the docx filename, its provenance header, etc."
    )
    status: JobStatus

    model_config = {"json_schema_extra": {"examples": [{"job_id": "a1b2c3d4", "status": "queued"}]}}
            ...
            "examples": [
                {
                    ...
                    "updated_at": "2026-09-25T10:00:03Z",
                    "error": None,
                }
            ]
        },
...
class SummaryOut(BaseModel):
    ...
    title: str = Field(
        description="Short, filename-safe title (see docx_filename in summarize.py)."
    )
            ...
            "examples": [
                {
                    ...
                    "summary": {
                        ...
                        "description": "Uma chamada curta confirmando o horário de uma reunião.",
                        "topics": ["reunião", "agenda"],
                        "speakers_detected": 1,
...
class HealthResponse(BaseModel):
    status: Literal["ok", "degraded"] = Field(
        description="'degraded' means at least one dependency (ffmpeg/ffprobe/Ollama) is unreachable; "  # noqa: E501
...
class ConfigPatch(BaseModel):
    ...
    llm_model: str | None = Field(
        default=None, description="Validated against `ollama list` before being accepted."
    )
    refine_prompt_id: str | None = Field(
        default=None, description="Must be a known id -- see prompts.py."
    )
    ...
    model_config = {"json_schema_extra": {"examples": [{"llm_model": "qwen3.5:4b"}]}}
...
class BenchmarkCreateRequest(BaseModel):
    ...
    tag: str = Field(
        description="A unique name for this run, e.g. 'qwen9b-baseline'. Re-used to fetch results."
    )
    n: int = Field(
        default=30, gt=0, description="Sample size for the 'quick' tier; ignored otherwise."
    )
    seed: int = Field(
        default=42, description="Sampling seed for the 'quick' tier, for reproducibility."
    )
    ...
    semdist: bool = Field(
        default=True, description="Compute SemDist (loads a sentence-embedding model on first use)."
    )

    model_config = {
        "json_schema_extra": {
            "examples": [{"dataset": "fleurs", "tier": "smoke", "tag": "qwen9b-baseline"}]
        }
    }
